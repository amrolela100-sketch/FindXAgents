...
router.get("/search/status", async (_req, res) => {
  const apiKey = await getTavilyApiKey();
  const [config] = await db.select().from(searchConfigs).where(eq(searchConfigs.id, "default")).catch(() => [null]);
  return res.json({
    configured: !!apiKey,
    provider: "tavily",
...
router.get("/search/config", async (_req, res) => {
  try {
    const [config] = await db.select().from(searchConfigs).where(eq(searchConfigs.id, "default"));
...
router.put("/search/config", async (req, res) => {
  ...
  try {
    const [existing] = await db.select().from(searchConfigs).where(eq(searchConfigs.id, "default"));
    if (existing) {
      await db.update(searchConfigs).set({ apiKey: parsed.data.apiKey, provider: parsed.data.provider, updatedAt: new Date() }).where(eq(searchConfigs.id, "default"));
    } else {
      await db.insert(searchConfigs).values({ id: "default", apiKey: parsed.data.apiKey, provider: parsed.data.provider });
    }
...
router.delete("/search/config", async (_req, res) => {
  try {
...
router.post("/search/test", async (_req, res) => {
  const apiKey = await getTavilyApiKey();
  if (!apiKey) return res.status(503).json({ ok: false, error: "Tavily API key not configured" });
