import { Users, Target, TrendingUp, Zap, ArrowRight } from "lucide-react";
import type { DashboardStats } from "../lib/types";
import { getDashboardStats } from "../lib/api";
import { useRealtimeData } from "../lib/hooks/use-realtime-data";

export function MetricsBar() {
  const { data } = useRealtimeData(() => getDashboardStats(), ["leads", "agent_pipeline_runs"], 30_000);
  ...
  const contactRate = totalLeads > 0 ? Math.round((leadsContacted / totalLeads) * 100) : 0;
  const responseRate = leadsContacted > 0 ? Math.round((leadsResponded / leadsContacted) * 100) : 0;
  const winRate = leadsResponded > 0 ? Math.round((leadsWon / leadsResponded) * 100) : 0;
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div>
          <p className={`text-xs font-medium ${highlight ? "text-white/70" : "text-slate-400"}`}>
            {label}
          </p>
          <p className={`text-2xl font-bold mt-1 ${highlight ? "text-white" : "text-slate-100"}`}>
            {value}
          </p>
          <p className={`text-xs mt-1 ${highlight ? "text-white/60" : "text-slate-400"}`}>
            {subtitle}
          </p>
        </div>
