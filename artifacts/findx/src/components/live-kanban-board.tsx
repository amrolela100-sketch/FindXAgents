...
function LiveColumn({
  ...
  return (
    <div
      ref={setNodeRef}
      className={`flex-shrink-0 w-[18rem] sm:w-72 bg-[#F0EDE6] rounded-xl border transition-all duration-300 ${
        isOver
    ...
    >
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-[#E5E3D9]">
        ...
        <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-white border border-[#E5E3D9] text-[#7A756D]">
          {count}
        </span>
      </div>
      ...
      <div className="overflow-y-auto max-h-[calc(100vh-340px)] p-2 space-y-2">
        {children}
        {count === 0 && (
          <div className="flex flex-col items-center justify-center py-8 border-2 border-dashed border-[#D9D6CE] rounded-lg">
            <span className="text-xs text-[#BDBDB0]">No leads</span>
          </div>
        )}
      </div>
    </div>
