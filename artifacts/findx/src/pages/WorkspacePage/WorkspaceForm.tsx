  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 pt-2">
        <button onClick={() => onSave(form)} disabled={saving || !valid} className="btn btn-primary text-[13px] px-4 py-2 gap-2 font-semibold">
          {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" strokeWidth={2.5} />} Save workspace
        </button>
        <button onClick={onCancel} className="btn btn-ghost text-[13px] px-4 py-2">Cancel</button>
      </div>
    </div>
  );
}
