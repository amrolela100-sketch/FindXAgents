...
export default function OnboardingPage({ onComplete }: { onComplete?: () => void }) {
  return (
    <div className="bg-base-cream text-text-ink font-body-lg min-h-screen flex flex-col antialiased">
      ...
      {/* ── Header ── */}
      <header className="w-full px-margin-page py-6 flex justify-between items-center bg-base-cream">
        <div className="font-headline-lg text-headline-lg text-text-ink tracking-tight flex items-center gap-2">
          <span className="material-symbols-outlined text-accent-gold" style={{ fontSize: 32 }}>
            storm
          </span>
          FindX
        </div>
        <div>
          <a
            className="font-label-caps text-label-caps text-text-ink hover:text-accent-gold transition-colors duration-300 mr-6"
            href="#"
          >
            Sign In
          </a>
      ...
      <main className="flex-grow flex flex-col items-center w-full max-w-7xl mx-auto px-4 sm:px-margin-page">
        {/* ── Hero ── */}
        <section className="w-full py-24 flex flex-col items-center text-center space-y-stack-md relative">
          <div className="max-w-4xl space-y-6">
            ...
            <h1 className="font-display-lg text-display-lg text-text-ink">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 pt-8">
            ...
            <button className="px-8 py-4 bg-surface-white border border-outline-variant text-text-ink font-label-caps text-label-caps rounded hover:bg-surface-container-low transition-colors duration-300 flex items-center justify-center gap-3 w-full sm:w-auto">
